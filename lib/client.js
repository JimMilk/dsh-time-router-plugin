...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function trendColor(provider) {
      if (provider.indexOf('scnet') !== -1) return 'tr-trend--scnet'
      if (provider.indexOf('deepseek') !== -1) return 'tr-trend--official'
      return 'tr-trend--other'
    }

    function trendName(provider) {
    ...
    function TrendChart(props) {
      ...
      return react.createElement('div', { className: 'tr-trend-wrap' },
        react.createElement('div', { className: 'tr-trend' },
          trend.map(function (d) {
            var segs = Object.keys(d.byProvider || {}).map(function (p) {
              ...
              return react.createElement('span', {
                key: p,
                className: 'tr-trend-seg ' + trendColor(p),
                style: { height: h + '%' },
        ...
        react.createElement('div', { className: 'tr-trend-legend' },
          providers.map(function (p) {
            return react.createElement('span', { key: p, className: 'tr-trend-legend-item' },
              react.createElement('span', { className: 'tr-trend-swatch ' + trendColor(p) }),
              ' ', trendName(p),
            )
          }),
        ),
    ...
    exports.apply = function (ctx) {
      ...
      if (typeof document !== 'undefined' && !document.getElementById('tr-style')) {
        ...
        style.textContent = [
          ...
          '.tr-trend--other{background:#9e9e9e}',
          '.tr-trend-date{font-size:10px;color:#57606a}',
          '.tr-trend-legend{display:flex;gap:12px;margin-top:4px;font-size:11px;color:#57606a;flex-wrap:wrap}',
          '.tr-trend-legend-item{display:inline-flex;align-items:center;gap:4px}',
          '.tr-trend-swatch{width:10px;height:10px;border-radius:2px;display:inline-block}',
